...
///
/// Important scope caveat, same one the desktop agents' own button carries:
/// forceEvaluateNow (compliance.service.ts) re-evaluates every enabled policy
/// against the ENTIRE workspace fleet, not just this one device — there is no
/// per-device evaluate-now endpoint. Pressing this button on one phone
/// re-checks compliance for every managed device in the workspace; this
/// device's own status card is simply the most immediate place to see the
/// result, via the status re-fetch the caller (compliance_screen.dart) does
...
class ComplianceActionsClient {
  ...
  Future<ComplianceEvaluationSummary> forceEvaluate(
    ...
    final url = Uri.parse(config.baseUrl).resolve('/api/device-data/evaluate-now');
